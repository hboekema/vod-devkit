...
from vod.utils.data_classes import Box
...
def load_prediction(result_path: str, max_boxes_per_sample: int, box_cls, verbose: bool = False) \
    ...
    assert 'results' in data, 'Error: No field `results` in result file. Please note that the result format changed.' \
                              'See https://www.vod.org/object-detection for more information.'
...
def load_gt(nusc: VOD, eval_split: str, box_cls, verbose: bool = False) -> EvalBoxes:
    """
    Loads ground truth boxes from DB.
    :param nusc: A VOD instance.
    ...
    if verbose:
        print('Loading annotations for {} split from VOD version: {}'.format(eval_split, nusc.version))
    ...
    if eval_split in {'train', 'val', 'train_detect', 'train_track'}:
        assert version.endswith('trainval'), \
            'Error: Requested split {} which is not compatible with VOD version {}'.format(eval_split, version)
        ...
        raise ValueError('Error: Requested split {} which this function cannot map to the correct VOD version.'
                         .format(eval_split))
    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == DetectionBox:
                ...
                # Use VOD token as tracking id.
                tracking_id = sample_annotation['instance_token']
                tracking_id_set.add(tracking_id)

                # Get label name in detection task and filter unused labels.
                # Import locally to avoid errors when motmetrics package is not installed.
                from vod.eval.tracking.utils import category_to_tracking_name
...
def add_center_dist(nusc: VOD,
                    eval_boxes: EvalBoxes):
    """
    Adds the cylindrical (xy) center distance from ego vehicle to each box.
    :param nusc: The VOD instance.
...
def filter_eval_boxes(nusc: VOD,
                      eval_boxes: EvalBoxes,
                      max_dist: Dict[str, float],
                      verbose: bool = False) -> EvalBoxes:
    """
    Applies filtering to boxes. Distance, bike-racks and points per box.
    :param nusc: An instance of the VOD class.
