...
Script to evaluate Panoptic VOD panoptic segmentation (PS) or panoptic tracking (PT) metrics.
...
---------------------------------------------------------
python python-sdk/vod/eval/panoptic/evaluate.py --result_path /data/panoptic_pred_results --eval_set mini_val
--task tracking --dataroot /data/sets/vod --version v1.0-mini --out_dir /tmp/panoptic_eval_output
---------------------------------------------------------
...
import numpy as np
from vod.eval.panoptic.panoptic_seg_evaluator import PanopticEval
from vod.eval.panoptic.panoptic_track_evaluator import PanopticTrackingEval
from vod.eval.panoptic.utils import PanopticClassMapper, get_samples_in_panoptic_eval_set
from vod.vod import VOD
from vod.utils.data_io import load_bin_file
from vod.utils.splits import create_splits_scenes
from tqdm import tqdm


class VODPanopticEval:
    """
    This is the official Panoptic VOD evaluation code. Results are written to the provided output_dir.
    Panoptic VOD uses the following metrics:
    ...
    - Multi-object Panoptic Tracking: we use the PAT (Panoptic Tracking) metric, which is defined as:
      ...
      Panoptic VOD: A Large-Scale Benchmark for LiDAR Panoptic Segmentation and Tracking 
    ...
    def save_result(self, results: Dict[str, Dict[str, Any]]) -> None:
        ...
        if self.verbose:
            print(f"======\nPanoptic VOD {self.task} evaluation for {self.eval_set}")
            print(json.dumps(results, indent=4, sort_keys=False))
            print("======")


def main():
    parser = argparse.ArgumentParser(description='Evaluate Panoptic VOD results.')
    ...
    parser.add_argument('--dataroot', type=str, default='/data/sets/vod', help='Default VOD data directory.')
    ...
    out_dir = args.out_dir if args.out_dir is not None else f'Panoptic-VOD-{args.version}'
    task = args.task
    ...
    nusc = VOD(version=args.version, dataroot=args.dataroot, verbose=args.verbose)

    evaluator = VODPanopticEval(nusc=nusc,
