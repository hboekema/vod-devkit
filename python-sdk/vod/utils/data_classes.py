...
from vod.utils.geometry_utils import transform_matrix, view_points
...
class LidarSegPointCloud:
    ...
    def load_points(self, path: str) -> None:
        ...
        if self.labels is not None:
            assert len(self.points) == len(self.labels), (
                "Error: There are {} points in the point cloud, "
    ...
    def render(
        ...
        ax.scatter(
            self.points[:, 0], self.points[:, 1], c=colors[self.labels], s=dot_size
        )
