...
def create_splits_logs(split: str, nusc: 'VOD') -> List[str]:
    """
    ...
    assert split in scene_splits.keys(), 'Requested split {} which is not a known VOD split.'.format(split)

    # Check compatibility of split with nusc_version.
    version = nusc.version
    if split in {'train', 'val', 'train_detect', 'train_track'}:
        assert version.endswith('trainval'), \
            'Requested split {} which is not compatible with VOD version {}'.format(split, version)
...
def create_splits_scenes(verbose: bool = False) -> Dict[str, List[str]]:
    ...
    - train/val/test: The standard splits of the VOD dataset (700/150/150 scenes).
