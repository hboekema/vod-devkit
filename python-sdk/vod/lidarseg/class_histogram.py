...
from vod import VOD
from vod.panoptic.panoptic_utils import get_frame_panoptic_instances, get_panoptic_instances_stats
...
def render_histogram(nusc: VOD,
                     sort_by: str = 'count_desc',
                     verbose: bool = True,
                     font_size: int = 20,
                     save_as_img_name: str = None) -> None:
    """
    Render two histograms for the given VOD split. The top histogram depicts the number of scan-wise instances
    ...
    # Get the statistics for the given VOD split.
...
def get_lidarseg_num_points_per_class(nusc: VOD, sort_by: str = 'count_desc') -> Dict[str, int]:
    """
    Get the number of points belonging to each class for the given VOD split.
    :param nusc: A VOD object.
...
def get_panoptic_num_instances_per_class(nusc: VOD, sort_by: str = 'count_desc') -> Dict[str, int]:
    """
    Get the number of scan-wise instances belonging to each class for the given VOD split.
    :param nusc: A VOD object.
    :param sort_by: How to sort the classes:
