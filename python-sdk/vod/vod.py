# View-of-Delft dev-kit, based on the VOD dev-kit.
...
# from vod.lidarseg.lidarseg_utils import (colormap_to_colors,
#                                         create_lidarseg_legend,
#                                         get_labels_in_coloring, get_stats,
#                                         paint_points_label, plt_to_cv2)
# from vod.panoptic.panoptic_utils import (get_frame_panoptic_instances,
#                                         get_panoptic_instances_stats,
#                                         paint_panop_points_label,
#                                         stuff_cat_ids)
from vod.utils.color_map import get_colormap
from vod.utils.data_classes import Box, LidarPointCloud, RadarPointCloud
from vod.utils.geometry_utils import (BoxVisibility, box_in_image,
                                      transform_matrix, view_points)
from vod.utils.map_mask import MapMask

PYTHON_VERSION = sys.version_info[0]

if not PYTHON_VERSION == 3:
    raise ValueError("VOD dev-kit only supports Python version 3.")


class VOD:
    """
    Database class for View-of-Delft to help query and retrieve information from the database.
    """

    def __init__(
        self,
        version: str = "v1.0",
        dataroot: str = "/data/sets/vod",
        verbose: bool = True,
        ...
        if verbose:
            print("======\nLoading VOD tables for version {}...".format(self.version))
        ...
        # Initialize VODExplorer class.
        self.explorer = VODExplorer(self)
...
class VODExplorer:
    """Helper class to list and visualize VOD data. These are meant to serve as tutorials and templates for
    working with the data."""

    def __init__(self, vod: VOD):
        self.vod = vod

    def get_color(self, category_name: str) -> Tuple[int, int, int]:
        """
        Provides the default colors based on the category names.
        This method works for the general VOD categories, as well as the VOD detection categories.
        """

        return self.vod.colormap[category_name]

    def list_categories(self) -> None:
        """Print categories, counts and stats. These stats only cover the split specified in vod.version."""
        print("Category stats for split %s:" % self.vod.version)

        # Add all annotations.
        categories = dict()
        for record in self.vod.sample_annotation:
    ...
    def list_attributes(self) -> None:
        ...
        for record in self.vod.sample_annotation:
            for attribute_token in record["attribute_tokens"]:
                att_name = self.vod.get("attribute", attribute_token)["name"]
    ...
    def list_scenes(self) -> None:
        ...
        def ann_count(record):
            count = 0
            sample = self.vod.get("sample", record["first_sample_token"])
            while not sample["next"] == "":
                count += len(sample["anns"])
                sample = self.vod.get("sample", sample["next"])
            return count

        recs = [
            (self.vod.get("sample", record["first_sample_token"])["timestamp"], record)
            for record in self.vod.scene
        ]

        for start_time, record in sorted(recs):
            start_time = (
                self.vod.get("sample", record["first_sample_token"])["timestamp"]
                / 1000000
            )
            length_time = (
                self.vod.get("sample", record["last_sample_token"])["timestamp"]
                / 1000000
                - start_time
            )
            location = self.vod.get("log", record["log_token"])["location"]
    ...
    def list_sample(self, sample_token: str) -> None:
        ...
        sample_record = self.vod.get("sample", sample_token)
        print("Sample: {}\n".format(sample_record["token"]))
        for sd_token in sample_record["data"].values():
            sd_record = self.vod.get("sample_data", sd_token)
            print(
        ...
        for ann_token in sample_record["anns"]:
            ann_record = self.vod.get("sample_annotation", ann_token)
            print(
    ...
    def map_pointcloud_to_image(
        ...
        cam = self.vod.get("sample_data", camera_token)
        pointsensor = self.vod.get("sample_data", pointsensor_token)
        pcl_path = osp.join(self.vod.dataroot, pointsensor["filename"])
        if pointsensor["sensor_modality"] == "lidar":
            if show_lidarseg or show_panoptic:
                gt_from = "lidarseg" if show_lidarseg else "panoptic"
                assert hasattr(
                    self.vod, gt_from
                ), f"Error: VOD-{gt_from} not installed!"
        ...
        im = Image.open(osp.join(self.vod.dataroot, cam["filename"]))

        # Points live in the point sensor frame. So they need to be transformed via global to the image plane.
        # First step: transform the pointcloud to the ego vehicle frame for the timestamp of the sweep.
        cs_record = self.vod.get(
        ...
        poserecord = self.vod.get("ego_pose", pointsensor["ego_pose_token"])
        pc.rotate(Quaternion(poserecord["rotation"]).rotation_matrix)
        pc.translate(np.array(poserecord["translation"]))

        # Third step: transform from global into the ego vehicle frame for the timestamp of the image.
        poserecord = self.vod.get("ego_pose", cam["ego_pose_token"])
        pc.translate(-np.array(poserecord["translation"]))
        pc.rotate(Quaternion(poserecord["rotation"]).rotation_matrix.T)

        # Fourth step: transform from ego into the camera.
        cs_record = self.vod.get("calibrated_sensor", cam["calibrated_sensor_token"])
        pc.translate(-np.array(cs_record["translation"]))
        ...
        if render_intensity:
            ...
            gt_from = "lidarseg" if show_lidarseg else "panoptic"
            semantic_table = getattr(self.vod, gt_from)

            if lidarseg_preds_bin_path:
                sample_token = self.vod.get("sample_data", pointsensor_token)[
                    "sample_token"
                ]
                ...
                if (
                    ...
                    lidarseg_labels_filename = osp.join(
                        self.vod.dataroot,
                        self.vod.get(gt_from, pointsensor_token)["filename"],
                    )
                else:
            ...
            coloring = depths
            print(
                f"Warning: There are no lidarseg labels in {self.vod.version}. Points will be colored according "
    ...
    def render_sample(
        ...
        :param show_panoptic: When set to True, the lidar data is colored with the panoptic labels. When set
            ...
            If show_lidarseg is True, show_panoptic will be set to False.
        """
        record = self.vod.get("sample", token)
        ...
        for channel, token in record["data"].items():
            sd_record = self.vod.get("sample_data", token)
    ...
    def render_ego_centric_map(
        ...
        # Get data.
        sd_record = self.vod.get("sample_data", sample_data_token)
        sample = self.vod.get("sample", sd_record["sample_token"])
        scene = self.vod.get("scene", sample["scene_token"])
        log = self.vod.get("log", scene["log_token"])
        map_ = self.vod.get("map", log["map_token"])
        map_mask = map_["mask"]
        pose = self.vod.get("ego_pose", sd_record["ego_pose_token"])
    ...
    def render_sample_data(
        ...
        if sensor_modality in ["lidar", "radar"]:
            sample_rec = self.vod.get("sample", sd_record["sample_token"])
            chan = sd_record["channel"]
            ref_chan = "LIDAR_TOP"
            ref_sd_token = sample_rec["data"][ref_chan]
            ref_sd_record = self.vod.get("sample_data", ref_sd_token)
            ...
            if sensor_modality == "lidar":
                if show_lidarseg or show_panoptic:
                    ...
                    pcl_path = osp.join(self.vod.dataroot, ref_sd_record["filename"])
                    pc = LidarPointCloud.from_file(pcl_path)
                else:
                    # Get aggregated lidar point cloud in lidar frame.
                    pc, times = LidarPointCloud.from_file_multisweep(
                        self.vod, sample_rec, chan, ref_chan, nsweeps=nsweeps
                    )
                velocities = None
            else:
                ...
                # point cloud.
                radar_cs_record = self.vod.get(
                    "calibrated_sensor", sd_record["calibrated_sensor_token"]
                )
                ref_cs_record = self.vod.get(
            ...
            if use_flat_vehicle_coordinates:
                ...
                pose_record = self.vod.get("ego_pose", ref_sd_record["ego_pose_token"])
            ...
            print(
                ...
                "from the ego vehicle instead.".format(self.vod.version)
            )
            ...
            _, boxes, _ = self.vod.get_sample_data(
                ref_sd_token,
                box_vis_level=box_vis_level,
                use_flat_vehicle_coordinates=use_flat_vehicle_coordinates,
            )
            ...
            data_path, boxes, camera_intrinsic = self.vod.get_sample_data(
    ...
    def render_annotation(
        ...
        """
        ann_record = self.vod.get("sample_annotation", anntoken)
        sample_record = self.vod.get("sample", ann_record["sample_token"])
        assert (
        ...
        assert len(boxes) > 0, (
            "Error: Could not find image where annotation is visible. "
        ...
        axes[1].imshow(im)
        axes[1].set_title(self.vod.get("sample_data", cam)["channel"])
        axes[1].axis("off")
        ...
        if extra_info:
            ...
            sample_data_record = self.vod.get(
                "sample_data", sample_record["data"]["LIDAR_TOP"]
            )
            pose_record = self.vod.get("ego_pose", sample_data_record["ego_pose_token"])
            dist = np.linalg.norm(
    ...
    def render_instance(
        ...
        :param extra_info: Whether to render extra information below camera view.
        """
        ann_tokens = self.vod.field2token(
    ...
    def render_scene(
        ...
        # Get records from DB.
        scene_rec = self.vod.get("scene", scene_token)
        first_sample_rec = self.vod.get("sample", scene_rec["first_sample_token"])
        last_sample_rec = self.vod.get("sample", scene_rec["last_sample_token"])
        ...
        for channel in layout:
            current_recs[channel] = self.vod.get(
        ...
        while current_time < last_sample_rec["timestamp"]:
            ...
            for channel, sd_rec in current_recs.items():
                while sd_rec["timestamp"] < current_time and sd_rec["next"] != "":
                    sd_rec = self.vod.get("sample_data", sd_rec["next"])
            ...
            for channel, sd_rec in current_recs.items():
                ...
                if not sd_rec == prev_recs[channel]:
                    ...
                    impath, boxes, camera_intrinsic = self.vod.get_sample_data(
    ...
    def render_scene_channel(
        ...
        assert channel in valid_channels, "Error: Input channel {} not valid.".format(
            channel
        )
        ...
        # Get records from DB.
        scene_rec = self.vod.get("scene", scene_token)
        sample_rec = self.vod.get("sample", scene_rec["first_sample_token"])
        sd_rec = self.vod.get("sample_data", sample_rec["data"][channel])
    ...
    def render_egoposes_on_map(
        ...
        log_tokens = [
            log["token"] for log in self.vod.log if log["location"] == log_location
        ]
        ...
        scene_tokens_location = [
            e["token"] for e in self.vod.scene if e["log_token"] in log_tokens
        ]
        ...
        for scene_token in tqdm(scene_tokens_location):
            ...
            scene_record = self.vod.get("scene", scene_token)
            log_record = self.vod.get("log", scene_record["log_token"])
            map_record = self.vod.get("map", log_record["map_token"])
            map_mask = map_record["mask"]

            # For each sample in the scene, store the ego pose.
            sample_tokens = self.vod.field2token("sample", "scene_token", scene_token)
    ...
    def _plot_points_and_bboxes(
        ...
        if with_anns:
            ...
            for box in boxes:
                ...
                if (
                    filter_lidarseg_labels is not None
                    and self.vod.lidarseg_name2idx_mapping[box.name]
                    not in filter_lidarseg_labels
                ):
                    continue
                c = self.get_color(box.name)
                box.render_cv2(
    ...
    def render_scene_channel_lidarseg(
        self,
        scene_token: str,
        channel: str,
        out_folder: str = None,
        ...
        assert hasattr(self.vod, gt_from), f"Error: VOD-{gt_from} not installed!"
        ...
        ), "Error: Input camera channel {} not valid.".format(channel)
        assert imsize[0] / imsize[1] == 16 / 9, "Error: Aspect ratio should be 16/9."

        if lidarseg_preds_folder:
            assert os.path.isdir(
                lidarseg_preds_folder
            ), "Error:  The lidarseg predictions folder ({}) does not exist.".format(
                lidarseg_preds_folder
            )

        save_as_vid = False
        if out_folder:
            assert render_mode in ["video", "image"], (
                "Error: For the renderings to be saved to {}, either `video` "
                "or `image` must be specified for render_mode. {} is "
                "not a valid mode.".format(out_folder, render_mode)
            )
            assert os.path.isdir(out_folder), "Error: {} does not exist.".format(
                out_folder
            )
        ...
        if verbose:
            name = "{}: {} {labels_type} (Space to pause, ESC to exit)".format(
                scene_record["name"],
                channel,
                labels_type="(predictions)" if lidarseg_preds_folder else "",
            )
            cv2.namedWindow(name)
            cv2.moveWindow(name, 0, 0)
        else:
        ...
        if save_as_vid:
            out_path = os.path.join(
                out_folder, scene_record["name"] + "_" + channel + ".avi"
            )
            fourcc = cv2.VideoWriter_fourcc(*"MJPG")
            out = cv2.VideoWriter(out_path, fourcc, freq, imsize)
        else:
        ...
        while keep_looping:
            ...
            sample_record = self.vod.get("sample", current_token)
            ...
            if lidarseg_preds_folder:
                if show_panoptic:
                    lidarseg_preds_bin_path = osp.join(
                        lidarseg_preds_folder, pointsensor_token + "_panoptic.npz"
                    )
                else:
                    lidarseg_preds_bin_path = osp.join(
                        lidarseg_preds_folder, pointsensor_token + "_lidarseg.bin"
                    )
            else:
                lidarseg_preds_bin_path = None

            mat, no_points_in_mat = self._plot_points_and_bboxes(
                pointsensor_token,
                camera_token,
                filter_lidarseg_labels=filter_lidarseg_labels,
                lidarseg_preds_bin_path=lidarseg_preds_bin_path,
        ...
        if save_as_vid:
            assert total_num_samples == i, (
                "Error: There were supposed to be {} keyframes, "
                "but only {} keyframes were processed".format(total_num_samples, i)
            )
            out.release()

    def render_scene_lidarseg(
        self,
        scene_token: str,
        out_path: str = None,
        filter_lidarseg_labels: Iterable[int] = None,
        ...
        if out_path:
            ...
            assert os.path.isdir(path_to_file), "Error: {} does not exist.".format(
                path_to_file
            )
            assert (
                os.path.splitext(filename)[-1] == ".avi"
            ), "Error: Video can only be saved in .avi format."
            fourcc = cv2.VideoWriter_fourcc(*"MJPG")
        ...
        while keep_looping:
            ...
            for camera_channel in layout:
                ...
                mat, _ = self._plot_points_and_bboxes(
                    pointsensor_token,
                    camera_token,
                    filter_lidarseg_labels=filter_lidarseg_labels,
                    lidarseg_preds_bin_path=lidarseg_preds_bin_path,
