...
"""
This script converts VOD data to KITTI format and KITTI results to VOD.
It is used for compatibility with software that uses KITTI-style annotations.

We do not encourage this, as:
- KITTI has only front-facing cameras, whereas VOD has a 360 degree horizontal fov.
- KITTI has no radar data.
- The VOD database format is more modular.
- KITTI fields like occluded and truncated cannot be exactly reproduced from VOD data.
- KITTI has different categories.

Limitations:
- We don't specify the KITTI imu_to_velo_kitti projection in this code base.
- We map VOD categories to VOD detection categories, rather than KITTI categories.
- Attributes are not part of KITTI and therefore set to '' in the VOD result format.
- Velocities are not part of KITTI and therefore set to 0 in the VOD result format.
- This script uses the `train` and `val` splits of VOD, whereas standard KITTI has `training` and `testing` splits.

This script includes three main functions:
- vod_gt_to_kitti(): Converts VOD GT annotations to KITTI format.
- render_kitti(): Render the annotations of the (generated or real) KITTI dataset.
- kitti_res_to_vod(): Converts a KITTI detection result to the VOD detection results format.

To launch these scripts run:
- python export_kitti.py vod_gt_to_kitti --nusc_kitti_dir ~/nusc_kitti
- python export_kitti.py render_kitti --nusc_kitti_dir ~/nusc_kitti --render_2d False
- python export_kitti.py kitti_res_to_vod --nusc_kitti_dir ~/nusc_kitti
...
See https://www.vod.org/object-detection for more information on the VOD result format.
"""
import json
import os
...
from vod.eval.detection.utils import category_to_detection_name
from vod.vod import VOD
from vod.utils.data_classes import LidarPointCloud, Box
from vod.utils.geometry_utils import BoxVisibility, transform_matrix
from vod.utils.kitti import KittiDB
from vod.utils.splits import create_splits_logs
...
class KittiConverter:
    def __init__(self,
        ...
        :param nusc_version: VOD version to use.
        ...
        self.nusc = VOD(version=nusc_version)

    def vod_gt_to_kitti(self) -> None:
        """
        Converts VOD GT annotations to KITTI format.
        """
        ...
        for sample_token in sample_tokens:
            ...
            # Note: cam uses same conventions in KITTI and VOD.
            ...
            # Convert from KITTI to VOD LIDAR coordinates, where we apply velo_to_cam.
            ...
            with open(label_path, "w") as label_file:
                for sample_annotation_token in sample_annotation_tokens:
                    ...
                    # Occluded: Set all objects to full visibility as this information is not available in VOD.
                    occluded = 0

                    # Convert VOD category to VOD detection challenge category.
                    detection_name = category_to_detection_name(sample_annotation['category_name'])

                    # Skip categories that are not part of the VOD detection challenge.
                    if detection_name is None:
                        continue

                    # Convert from VOD to KITTI box format.
                    box_cam_kitti = KittiDB.box_vod_to_kitti(
    ...
    def kitti_res_to_vod(self, meta: Dict[str, bool] = None) -> None:
        """
        Converts a KITTI detection result to the VOD detection results format.
        :param meta: Meta data describing the method used to generate the result. See vod.org/object-detection.
        """
        ...
        for sample_token in sample_tokens:
            ...
            # Convert KITTI boxes to VOD detection challenge result format.
