...
jupyter nbconvert --to python python-sdk/tutorials/vod_tutorial.ipynb || { echo "Failed to convert vod_tutorial notebook to python script"; exit 1; }
...
sed -i.bak "/get_ipython.*/d; s/\(nusc\.render.*\)/#\1/" python-sdk/tutorials/vod_tutorial.py || { echo "error in sed command"; exit 1; }
...
# Run tutorial
xvfb-run python python-sdk/tutorials/vod_tutorial.py
